...
float legPos;
int rot = 0;
int rotSwitch = 0;      // end switch for rotational axis
...
void setup() {
  ...
  pinMode(2, OUTPUT);     // rot motor PWMs
  pinMode(3, OUTPUT);
  pinMode(4, OUTPUT);     // leg motor PWMs
  pinMode(5, OUTPUT);
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {   // start of timed loop
      ...
      angle = angle -220 - 17; // calibrate for zero
      ...
      if (Serial1.available() > 0){           // receive IMU data from serial IMU
        ...
        marker = Serial1.parseInt();
          if (marker == 9000) {                   // look for check value to check it's the start of the data
              ...
              rot = Serial1.parseInt();
              if (Serial1.read() == '\n') {     // end of IMU data 
              }
          }
     }

     rotSwitch = digitalRead(6);
     ...
     else if (rot == 2 && rotSwitch == 1) {  // closing
        analogWrite(2, 160);
        analogWrite(3, 0);
     }
     else {
