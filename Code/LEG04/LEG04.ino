...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {   // start of timed loop
      ...
      angle = angle -208 + 17; // calibrate for zero
      angle = angle *- 1;     // reverse for legs 1 and 4

      if (Serial2.available() > 0){           // receive IMU data from serial IMU
        //Serial.println("data");
        marker = Serial2.parseInt();
          if (marker == 9000) {                   // look for check value to check it's the start of the data
              jog = Serial2.parseInt();
              def = Serial2.parseInt();
              walk = Serial2.parseInt();
              legJog1 = Serial2.parseInt();
              legJog2 = Serial2.parseInt();
              stick = Serial2.parseInt();
              if (Serial2.read() == '\n') {     // end of IMU data 
              }
          }
     }
